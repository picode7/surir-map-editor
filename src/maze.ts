...
function wallWithinBounds(
	...
	if (horizontal) {
		if (
			inBounds(
				{ x: wall.position.x1, y: wall.position.y1 },
				{ x1: 0, y1: 1, x2: size.width - 1, y2: size.height - 1 }
			) === false
		)
			return false
	} else {
		if (
			inBounds(
				{ x: wall.position.x1, y: wall.position.y1 },
				{ x1: 1, y1: 0, x2: size.width - 1, y2: size.height - 1 }
			) === false
		)
			return false
	}
...
function getWallAt<T extends { position: Line }>(
	walls: T[],
	position: Line
): T | null {
...
function generateMazeData(
	width: number,
	height: number,
	walls: { position: Line; type: WallType }[]
) {
...
function importSIRFile(text: string) {
	...
	let fogColor: number[] = []
	let floorTexture: string | undefined
	let skyTexture: string | undefined
	let doorTexture: string | undefined
	let wallTexture: string | undefined
		...
		if (line[0] == '$') {
			...
			switch (m) {
				case 0:
					...
					break
				case 1:
					titleColor[0] = parseInt(param) /*title color r*/
					break
				case 2:
					titleColor[1] = parseInt(param) /*title color g*/
					break
				case 3:
					titleColor[2] = parseInt(param) /*title color b*/
					break
				case 4:
					author = param
					break
				...
				case 8:
					fogColor = [
						parseInt(param.substr(0, 3)),
						parseInt(param.substr(3, 3)),
						parseInt(param.substr(6, 3)),
					] /*fog color rgb*/
					break
				case 9:
					floorTexture = fParseTexOrColor(param)
					break
				case 10:
					skyTexture = fParseTexOrColor(param)
					break
				case 11:
					doorTexture = fParseTexOrColor(param)
					break
				case 12:
					wallTexture = fParseTexOrColor(param)
					break
			}
			m++
		} else {
...
function generateMazeDataInterlaced(
	width: number,
	height: number,
	walls: { position: Line; type: WallType }[]
) {
